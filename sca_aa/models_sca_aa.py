...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self,hidden_size, vocab_size, att_dim, embed_size, encoded_dim):
        ...
        self.init_weights()
        self.c_linear_v = nn.Linear(1, 512)
    ...
    def channel_attention(self, V, h):
        ...
        return torch.squeeze(beta, 2)  # [batch, 2048]
    ...
    def forward(self, enc_image, global_features, encoded_captions, caption_lengths):
        ...
        global_features = global_features[sort_ind]             # (batch_size, embed_size) with sorted batches
        ...
        for timestep in range(max(decode_lengths)):
            ...
            b_c = self.channel_attention(global_features[:batch_size_t], h[:batch_size_t])
            global_tmp = b_c * global_features[:batch_size_t]
            global_tmp = F.relu(self.global_features(global_tmp[:batch_size_t]))      # (batch_size_t,embed_size)
            ...
            embeddings = embeddings[:batch_size_t]
            global_tmp = global_tmp.unsqueeze(1).expand_as(embeddings)
            inputs = torch.cat((embeddings,global_tmp), dim = 2)    # (batch_size_t, max_caption_length, embed_dim * 2)
