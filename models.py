...
class AdaptiveAttention(nn.Module):
    ...
    def forward(self, spatial_image, decoder_out, st):
        ...
        hidden_affine = F.tanh(self.h_affine(decoder_out))    # (batch_size,hidden_size)  # LSTM输出的隐藏层ht
        ...
        attention = F.tanh(attended_features + hidden_resized)    # (batch_size, num_pixels+1, att_dim)  # 过完att并拼接的feature和过完att的ht直接相加，过tanh
        ...
        out_l = F.tanh(self.context_hidden(context + hidden_affine))  # 这是什么输出？我们接着看！
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self,hidden_size, vocab_size, att_dim, embed_size, encoded_dim, att_mode=None):
        ...
        self.s_linear_v = nn.Linear(2048, 512)  # out=[batch, 49, 512]
        self.s_linear_h = nn.Linear(hidden_size, 512, bias=False)
        self.s_linear_a = nn.Linear(512, 1)  # out=[batch, 49, 1]

        self.c_linear_v = nn.Linear(1, 512)  # out=[batch, 2048, 512]
        self.c_linear_h = nn.Linear(hidden_size, 512, bias=False)
        self.c_linear_b = nn.Linear(512, 1)  # out=[batch, 2048, 1]
    ...
    def spatial_attention(self, V, h):
        ...
        :param V: shape=[batch_size, pixels_num=49, feature_num]
        :param h: shape=[batch_size, hidden_size]
        :return alpha: shape=[batch_size, pixels_num, 1]
        """
        v_feats = self.s_linear_v(V)
        h_feats = self.s_linear_h(h).view(-1, 1, self.k)  # [batch, 49, k]
        a = F.tanh(v_feats + h_feats)  # [batch, 49, k]
        alpha = F.softmax(self.s_linear_a(a), dim=1)  # [batch, 49, 1]
        return alpha

    def channel_attention(self, V, h):
        """
        calculate the channel-wise attention weights
        :param V: shape=[batch_size, 49, 2048]
        :param h: shape=[batch_size, hidden_size]
        :return beta: shape=[batch_size, 2048, 1]
        """
        ...
        b = F.tanh(v_feats + h_feats)  # [batch, 2048, k]
        beta = F.softmax(self.c_linear_b(b), dim=1)  # [batch, 2048, 1]
    ...
    def forward(self, enc_image, global_features, encoded_captions, caption_lengths):
        ...
        for timestep in range(max(decode_lengths)):
            ...
            # spatial_image的attention处理
            spatial_image = enc_image.clone()
            batch_size_t = sum([l > timestep for l in decode_lengths])  # 句子长度比当前时间步长，才有意义继续产生单词
            if self.att_mode == 'cs':
                b_c = self.channel_attention(spatial_image[:batch_size_t], h[:batch_size_t])
                spatial_image[:batch_size_t] = b_c * spatial_image[:batch_size_t]
                a_s = self.spatial_attention(spatial_image[:batch_size_t], h[:batch_size_t])
                spatial_image[:batch_size_t] = a_s * spatial_image[:batch_size_t]
            ...
            spatial_image = F.relu(self.encoded_to_hidden(spatial_image))  # (batch_size, num_pixels, hidden_size)  # 第三维由feature_num变成 hidden_size
