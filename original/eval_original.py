...
word_map_file = 'input_files/WORDMAP_flickr8k_5_cap_per_img_5_min_word_freq.json'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
def evaluate(beam_size):
    ...
    for i, (image, caps, caplens, allcaps) in enumerate(
        ...
        while True:
            b_c = decoder.channel_attention(enc_image, h)
            spatial_image = b_c * enc_image
            a_s = decoder.spatial_attention(spatial_image, h)
            spatial_tmp = a_s * spatial_image
            spatial_tmp = F.relu(decoder.encoded_to_hidden(spatial_tmp))  # (batch_size, num_pixels, hidden_size)  # 第三维由feature_num变成 hidden_size 
            ...
            out_l, _, _ = decoder.adaptive_attention(spatial_tmp, h, st)
