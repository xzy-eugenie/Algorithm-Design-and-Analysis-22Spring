...
class DecoderWithAttention(nn.Module):
    ...
    def spatial_attention(self, V, h):
        ...
        h_feats = self.s_linear_h(h).view(-1, 1, 512)  # [batch, 49, k]
        a = torch.tanh(v_feats + h_feats)  # [batch, 49, k]
    ...
    def channel_attention(self, V, h):
        ...
        h_feats = self.c_linear_h(h).view(-1, 1, 512)  # [batch, 1, k]
        b = torch.tanh(v_feats + h_feats)  # [batch, 2048, k]
    ...
    def forward(self, enc_image, global_features, encoded_captions, caption_lengths):
        ...
        for timestep in range(max(decode_lengths)):            
            ...
            b_c = self.channel_attention(enc_image[:batch_size_t], h[:batch_size_t])
            spatial_image = b_c * enc_image[:batch_size_t]
            a_s = self.spatial_attention(spatial_image, h[:batch_size_t])
            spatial_tmp = a_s * spatial_image
            spatial_tmp = F.relu(self.encoded_to_hidden(spatial_tmp))  # (batch_size, num_pixels, hidden_size)  # 第三维由feature_num变成 hidden_size
            ...
            out_l, alpha_t, beta_t = self.adaptive_attention(spatial_tmp,h,st)
