...
from models_original import Encoder, DecoderWithAttention
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Decay learning rate if there is no improvement for 8 consecutive epochs, and terminate training
        if epochs_since_improvement == 8:
            print("No Improvement. Training Terminated")
            break
        if epochs_since_improvement > 0 and epochs_since_improvement % 4 == 0:  # 效果已经没有提升, 修改学习率
            adjust_learning_rate(decoder_optimizer, 0.8)
            if fine_tune_encoder:  # encoder也要训练, 则encoder学习率也要修改
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        scores, alphas, betas, encoded_captions, decode_lengths, sort_ind = decoder(enc_image, global_features, 
                                                                                         caps, caplens)
        ...
        # loss += alpha_c * ((1. - alphas.sum(dim=1)) ** 2).mean()  # TODO: 这行是必需的吗？
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            # Add doubly stochastic attention regularization
            # loss += alpha_c * ((1. - alphas.sum(dim=1)) ** 2).mean()  # TODO: 这一行是必需的吗？
